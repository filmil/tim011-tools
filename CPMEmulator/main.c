...
/*
Da bi kod mogao da se kompajlira treba dodati pakete:
- za Debian/Ubuntu bazirane distribucije: libx11-dev libxext-dev
...
/* pauza koja se napravi kod svakog pristupa video memoriji, za snizavanje brzine simulacije*/
const int scriodelay = 50; /* 20 */
/* tekuca vrednost scroll registra TIM-a, IO 00D0H */
unsigned char scrscroll = 0;
/*********** TIM-011 END **********************/
...
/*********** TIM-011 BEGIN **********************/
    ...
    if (haddr >= 0x80) {
        ...
        *val = screen[ioaddr-0x8000];
/*        printf("IN %04X:%02X\n", ioaddr, screen[ioaddr-0x8000]);*/
        return TRUE;
        usleep(scriodelay);
    }
    else if (laddr == 0xD0) {
...
/*********** TIM-011 END **********************/
	...
	default:
		resetterm();
		printf("INPUT : addr = %02X%02X    DATA = ", haddr, laddr);
		fflush(stdout);
		scanf("%x", &data);
		setterm();
		*val = data;
...
/*********** TIM-011 END **********************/
	...
	if (laddr == 0xFF) {
		...
		printf("OUTPUT: addr = %02X%02X  DATA = %X\r\n", haddr, laddr,data);
	}
}
...
enum Colors {ColBlack = 0x00000000, ColRed=0x00308030, ColGreen=0x0050C050, ColBlue=0x0070ff70, ColWhite=0x00ffffff};
...
/*********** TIM-011 moved from original main() code into a thread **********************/
char cmd[256];
void *z80main(void *param) {
	if (cmd[0]) {
		stuff_cmd = cmd;
	}
...
/*    // kreiranje prozora*/
    s = DefaultScreen(d);
    r = RootWindow(d, s);
    w = XCreateSimpleWindow(d, r, 10, 10, scrwidth*SCALINGX, scrheight*SCALINGY, 0, ColBlack , ColBlack);
    XSelectInput(d, w, ExposureMask | KeyPressMask);
    XMapWindow(d, w);
    XFlush(d);
    XStoreName(d, w, "TIM Simulacija");
...
/*    // bafer za iscrtavanje*/
    DblBuffer = XCreatePixmap(d, w, scrwidth*SCALINGX, scrheight*SCALINGY, DefaultDepth(d,s));
...
/*    // pokretanje niti za emulirani ekran*/
    pthread_create(&testing, NULL, z80main, &parameter);
    pthread_create(&refresh, NULL, xrefresh, &parameter);
...
/*        // kopiranje bafera na sadržaj prozora*/
        if (e.type == Expose) {
            XFillRectangle(d, DblBuffer, GCCol[GCBlack], 0, 0, scrwidth*SCALINGX, scrheight*SCALINGY);
            for (i=0;i<scrwidth;i++) {
                for (j=0;j<scrheight;j++) {
                    offset = ((i/4) << 8) + ((j + scrscroll) & 0xFF);
                    col = (screen[offset]>>((i&3)*2))&3;
                    XFillRectangle(d, DblBuffer, GCCol[col], i*SCALINGX, j*SCALINGY, SCALINGX, SCALINGY);
                }
            }
            XCopyArea(d, DblBuffer, w, GCCol[GCBlack], 0, 0, scrwidth*SCALINGX, scrheight*SCALINGY, 0, 0);
        }
    }
